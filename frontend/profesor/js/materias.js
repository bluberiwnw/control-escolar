document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!materias.length) {
        container.innerHTML = '<div class="empty-state">No tienes materias asignadas.</div>';
        return;
    }
    container.innerHTML = materias
        ...
        <div class="course-card course-card--elevated">
            ...
            <div class="course-footer course-footer--split">
                <button type="button" class="btn btn-primary btn-sm" onclick="verMateria(${m.id})">Ver detalles</button>
            </div>
        </div>
    `
        )
