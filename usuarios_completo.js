...
async function cargarProfesores() {
    try {
        ...
        console.log('Ô£à Profesores cargados:', profesores.length);
        
        // Tambi├®n guardar todos los usuarios para uso compartido
        if (Array.isArray(lista) && lista.length > 0) {
            console.log('­ƒôª Guardando todosLosUsuarios en localStorage:', lista.length, 'usuarios');
            lista.forEach((usuario, index) => {
                console.log(`  ­ƒôØ Usuario ${index + 1} guardado:`, {
                    id: usuario.id,
            ...
            console.log('Ô£à Datos guardados exitosamente');
        }
        ...
        container.innerHTML = `<div class="table-responsive-wrap"><table class="data-table"><thead><tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Contrase├▒a</th><th>Acciones</th></tr></thead><tbody>
            ${profesores
                .map(
                    (p) => `<tr>
                ...
                <td data-label="Acciones" class="table-actions">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" onclick="cambiarContrasena(${p.id}, 'profesor')">Cambiar contrase├▒a</button>
...
async function cargarAdministradores() {
    try {
        ...
        try {
            ...
            console.log('Ô£à Administradores cargados directamente:', administradores.length);
        } catch (error) {
            console.log('ÔØî Error cargando administradores directamente:', error.message);
            ...
            console.log('Ô£à Administradores extraidos de datos compartidos:', administradores.length);
        }
        ...
        container.innerHTML = `<div class="table-responsive-wrap"><table class="data-table"><thead><tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Contrase├▒a</th><th>Acciones</th></tr></thead><tbody>
            ${administradores
                ...
                <td data-label="Acciones" class="table-actions">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" onclick="cambiarContrasena(${a.id}, 'administrador')">Cambiar contrase├▒a</button>
        ...
        const container = document.getElementById('listaAdministradores');
        if (container) {
            container.innerHTML = '<p class="alert alert-error">No se pudieron cargar los administradores. Intenta recargar la p├ígina.</p>';
        }
        mostrarToast('Error al cargar administradores. Verifica tu conexi├│n e intenta de nuevo.', 'error');
    }
}
...
async function cargarEstudiantes() {
    try {
        ...
        console.log('­ƒöì Verificaci├│n inicial de localStorage:');
        ...
        if (todosLosUsuarios.length > 0) {
            console.log('­ƒöä Usando datos guardados de todosLosUsuarios:', todosLosUsuarios.length);
            console.log('­ƒôè An├ílisis de usuarios totales:');
            ...
            // An├ílisis de roles ├║nicos para identificar posibles estudiantes
            const rolesUnicos = [...new Set(todosLosUsuarios.map(u => u.rol))];
            console.log('­ƒöì Roles ├║nicos encontrados:', rolesUnicos);
            
            estudiantes = todosLosUsuarios.filter(usuario => {
                // B├║squeda exhaustiva de estudiantes
                ...
                console.log(`  ­ƒöì An├ílisis usuario ${usuario.nombre}:`, {
                    rol: usuario.rol,
            ...
            console.log('Ô£à Estudiantes extraidos de datos guardados:', estudiantes.length);
        } else {
            console.log('ÔÜá´©Å No hay datos guardados, intentando endpoint directo...');
            
            // Como ├║ltimo recurso, intentar cargar con el m├®todo que funciona para profesores
            try {
                const lista = await apiRequest('/admin/usuarios?rol=profesor');
                const todosUsuarios = Array.isArray(lista) ? lista : [];
                
                if (todosUsuarios.length > 6) {
                    console.log('Ô£à Usando endpoint de profesores para extraer estudiantes...');
                    estudiantes = todosUsuarios.filter(usuario => {
                        return usuario.rol === 'alumno' || 
                               ...
                               usuario.role === 'estudiante' ||
                               usuario.tipo === 'estudiante' ||
                               // Tambi├®n verificar si no es profesor ni administrador (asumir que es estudiante)
                    ...
                    console.log('Ô£à Estudiantes extraidos de endpoint de profesores:', estudiantes.length);
                } else {
                    console.log('ÔØî Endpoint de profesores no contiene todos los usuarios');
                }
            } catch (error) {
                console.log('ÔØî Error en endpoint de respaldo:', error.message);
            }
        }
        ...
        if (estudiantes.length === 0) {
            console.log('­ƒôè Resumen de usuarios:', {
            ...
            container.innerHTML = `
                <div class="empty-state">
                    <h3>­ƒÄô No hay estudiantes registrados</h3>
                    ...
                    <p style="margin-top: 20px;">
                        <strong>Para agregar estudiantes:</strong> Usa el bot├│n "Nuevo estudiante" arriba.
                    </p>
                </div>
            `;
            return;
        ...
        container.innerHTML = `<div class="table-responsive-wrap"><table class="data-table"><thead><tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Contrase├▒a</th><th>Acciones</th></tr></thead><tbody>
            ${estudiantes
                ...
                <td data-label="Acciones" class="table-actions">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" onclick="cambiarContrasena(${e.id}, 'alumno')">Cambiar contrase├▒a</button>
        ...
        const container = document.getElementById('listaEstudiantes');
        if (container) {
            container.innerHTML = '<p class="alert alert-error">No se pudieron cargar los estudiantes. Intenta recargar la p├ígina.</p>';
        }
        mostrarToast('Error al cargar estudiantes. Verifica tu conexi├│n e intenta de nuevo.', 'error');
    }
}
...
async function editarProfesor(id, nombreActual, emailActual) {
    ...
    if (!esCorreoValido(email)) {
        mostrarToast('Ingresa un correo electr├│nico v├ílido', 'error');
        return;
    }
...
async function editarEstudiante(id, nombreActual, emailActual) {
    // Crear un formulario de edici├│n simple
    ...
    try {
        ...
        if (error.message.includes('duplicate key') || error.message.includes('already exists')) {
            if (error.message.includes('email')) {
                mostrarToast('El correo electr├│nico ya est├í registrado', 'error');
            } else {
...
async function guardarEstudiante(ev) {
    ...
    try {
        ...
        // Usar el mismo m├®todo que funciona para profesores
        try {
            console.log('­ƒöä Intentando crear estudiante con m├®todo de profesores...');
            ...
            console.log('Ô£à Estudiante creado exitosamente');
        } catch (error) {
            console.log('ÔØî Error creando estudiante con m├®todo de profesores:', error.message);
            
            // Si falla, intentar con rol 'estudiante'
            try {
                console.log('­ƒöä Intentando con rol estudiante...');
                await apiRequest('/admin/usuarios', {
                    method: 'POST',
                    body: JSON.stringify({
                        nombre,
                ...
                console.log('Ô£à Estudiante creado con rol estudiante');
            } catch (error2) {
                console.log('ÔØî Error con rol estudiante:', error2.message);
                throw new Error('No se pudo crear el estudiante. El servidor no est├í disponible o los endpoints no existen.');
            }
        }
        ...
        if (error.message.includes('duplicate key') || error.message.includes('already exists')) {
            ...
            mostrarToast('Datos inv├ílidos. Verifica toda la informaci├│n.', 'error');
...
// Funci├│n de validaci├│n para matr├¡cula
function esMatriculaValida(matricula) {
    return /^[0-9]{8}$/.test(matricula);
}

// Funci├│n para obtener nombre de carrera
function getCarreraNombre(carrera) {
    const carreras = {
        'computacion': 'Ciencias de la Computaci├│n',
        'software': 'Ingenier├¡a de Software',
        ...
        ">
            ...
            ">
                <h3 style="
                    ...
                    align-items: center;
                    gap: 10px;
                "><i class="fas fa-key"></i> Cambiar Contrase├▒a</h3>
                ...
                <form onsubmit="handleCambiarContrasena(event, ${userId}, '${tipo}')">
                    ...
                    ">
                        ...
                        <button type="submit" style="
                            ...
                            align-items: center;
                            gap: 8px;
                        "><i class="fas fa-save"></i> Actualizar Contrase├▒a</button>
                    </div>
                </form>
            </div>
        </div>
...
async function handleCambiarContrasena(event, userId, tipo) {
    ...
    if (!nuevaContrasena) {
        mostrarToast('Por favor ingresa la nueva contrase├▒a', 'error');
        return;
    }
    ...
    try {
        ...
        // Actualizar cache de usuarios con nueva contrase├▒a
        const usuariosCache = JSON.parse(localStorage.getItem('usuariosCache') || '[]');
        ...
        // Actualizar contrase├▒a en el DOM si est├í visible
        const passElement = document.getElementById(`pass-${userId}`);
        if (passElement) {
        ...
        mostrarToast('Error al actualizar contrase├▒a: ' + (error.message || 'Intenta de nuevo'), 'error');
    }
}
